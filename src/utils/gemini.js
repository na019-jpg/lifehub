...
4. **체류시간 극대화**: 사용자가 정보를 끝까지 읽도록 유도하여 광고 노출 및 클릭률(CTR)을 높입니다.
...
3. **구조 (AEO & SEO 결합)**:
   ...
   - [AEO 다이렉트 답변]: 서론 직후, 독자가 가장 궁금해할 핵심 질문에 대한 결론을 1~2문장으로 명확히 제시 (AI가 스니펫으로 긁어가기 좋은 형태)
   - [본문]: 최소 3개 이상의 소제목(H2, H3)으로 구분된 논리적 설명. 데이터 신뢰도를 높이는 표(Table)와 리스트(List) 적극 활용.
...

{
  ...
  "htmlContent": "<h1>제외, <h2>부터 시작하는 티스토리 블로그 본문 전체 HTML 내용 (1500자 이상). <h2>, <h3>, <table>, <ul>, <strong> 등을 적극 사용하여 아주 풍성하고 길게 작성하세요. 서론 직후에 '핵심 요약(AEO)' 섹션을 명확히 넣고, 하단에는 FAQ 섹션을 반드시 포함하세요. 글 중간중간에 [🖼️ 추천 이미지: 어떤 이미지] 또는 [💰 추천 광고 배치: 어떤 형태] 같은 텍스트 가이드도 넣어주세요."
}
`;
